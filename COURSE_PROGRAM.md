...
- [`eli5` skill](https://x.com/trq212/status/2090884855798407576) — **the example `SKILL.md` shown in class.** Nine lines including frontmatter, and the whole value is in a `description` that says what it does *and* when to fire. Found on Twitter, installed as a plugin, and run over this repo's own `COURSE_PROGRAM.md` to produce [a visual explainer](https://claude.ai/code/artifact/977c4128-625f-42c0-a78d-02e4425a887b) — the shortest possible path from "saw a skill" to "used it on my own material". Caveat to state in the room: it is run on Claude Code and **not yet verified on Pi**, because its body asks for an HTML artifact, which is a Claude Code capability. The format travels; the tools the body assumes may not. That nuance sharpens the open-standard line rather than undermining it.
...
- Guest: **Ale Silva** (CCAD) — the center, its hardware, getting access as a UNC student, running an LLM on it, and the inference stack (LiteLLM in front, vLLM behind). Details in the email thread; see `sessions/session-6/INSTRUCTOR.md` → *Pendiente de Ale*
