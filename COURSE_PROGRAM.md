...
**Guest: Ale Silva on CCAD, LiteLLM and vLLM (~30 min)**
...
**Theory: open-weight models (~30 min, Diego)** — built on [Flavio Copes's deep dive](https://flaviocopes.com/open-weight-models/), which runs the topics in the order the block needs.
...
- What a download actually gets you: safetensors vs. GGUF, tokenizer, runtime, and the token loop that never calls a cloud API. Then the **OpenAI-compatible endpoint**, which is why any harness can be pointed anywhere — the bridge to `models.json`.
- What it takes to run one: what a download actually gets you (safetensors vs. GGUF, tokenizer, runtime, the token loop), the quantization arithmetic (30B × 16 bits = 60 GB, × 4 bits = 15 GB), and the OpenAI-compatible endpoint as the reason a model swap is five lines of JSON. **Dense vs. MoE and the VRAM arithmetic on our own models were cut** with the move to Copes's arc.
- **`models.json`: the model as config** — the block the session turns on, now walked **at the top of the hands-on** with the room typing along, not in the theory. Field by field: `api: openai-completions` as the interoperability story typed out, `apiKey` as an env var and never a literal, and `contextWindow` as a number somebody chose.
- **Agus's portable GPU** is no longer a demo slot: it becomes the endpoint of the optional track, so the room connects to it rather than watching it.
...
- *After* the hands-on: **is it good enough to be the engine of a serious project?** — answered from their own measurements (schema-respecting tool calls, 20-step tasks, context window, latency in the loop), then when open source is the right call. Both deliberately placed once students have measured something themselves.

**Hands-on (~50-55 min), in two tracks**
- **Track A, everyone (~32 min)**: add the CCAD provider to `models.json`, pick the model with `/model`, and run a multi-step task **in their own repo with their own `AGENTS.md` and skills from Session 3** — then the same task on the hosted model, and compare tool-call validity, turns taken, hallucinated names, latency. Nothing here can be lost to a queue, so everyone finishes.
- **Track B, the advanced one, optional (~20 min)**: **Agus serves a small model on his GPU in the room, with Ollama, and students connect to it** — another provider entry pointing at his IP, nothing installed. It's the same swap a second time, it makes the "your own hardware" column physical, and it gives the comparison a third data point. The risk is concurrency: one GPU for the room queues, which is exactly what the guest explained about batching half an hour earlier — agree with Agus beforehand whether that's managed or used as the demonstration. Serving it on your own machine moves to a written appendix, with [Raschka's walkthrough](https://magazine.sebastianraschka.com/p/using-local-coding-agents) as the guide.
- **Extension** for whoever finished Session 5: point their own loop at the same endpoint. One base URL, no tunnel.

> **Timing is settled: this session runs 2 h 30.** Three big blocks — guest 30, open-weight models 30, hands-on 60 — plus recap, break, a 5-minute harvest of the measurements, and the 15-minute course closing. That is 150 minutes exactly, with no slack, so both 30s have to be rehearsed against a clock. Confirm the room for the extra half hour.
...
- [**Qwen3.8-27B**](https://huggingface.co/Qwen/Qwen3.8-27B) — **asked of Ale; a Qwen has run on CCAD before.** Qwen3.6 was tested working there and later taken down, so serving a Qwen on that hardware is proven — but 3.6 is what was proven, and 3.8 is what's being asked for. 27B **dense**, **Apache 2.0**, `bfloat16`, **262,144** native context, multimodal (unused here), tool-capable chat template, **ungated** repo. Those specs come from the model card, verified 2026-08-24 — not from seeing it served. The reason for asking is now weaker than it was: a second open model on the same endpoint gives the hands-on one more point of comparison. The strong reason — letting the room measure dense against MoE — went away with that content.
...
- [**Ollama**](https://ollama.com/) — **how Agus serves the classroom endpoint.** `ollama pull <model>`, then `OLLAMA_HOST=0.0.0.0:11434 ollama serve`. It exposes an OpenAI-compatible endpoint on `/v1`, so the room reaches it with a provider entry in `models.json` exactly as they reach CCAD — both tracks end up being the same operation against different endpoints, with nothing runtime-specific on the student's side. The three variables that matter: `OLLAMA_HOST` so the room can reach it at all, `OLLAMA_CONTEXT_LENGTH` for the window, and `OLLAMA_NUM_PARALLEL` for how many requests it serves at once — which is the knob behind the classroom concurrency risk. Verify the defaults before class.
- [**Sebastian Raschka: *Using Local Coding Agents***](https://magazine.sebastianraschka.com/p/using-local-coding-agents) — **the setup reference for the advanced local track**, built on **Ollama** rather than raw `llama-server` (one installer across Mac/Linux/Windows, OpenAI-compatible endpoint on `http://127.0.0.1:11434/v1`). Also points several open-source harnesses at that endpoint, which is the same move as our `models.json` verified by a third party, and carries measurements — 4-5/5 on agentic reasoning tasks with a Qwen3.6 MoE, ~40 tok/s on a Mac Mini, up to 30 GB RAM at 50k context — that feed the *is it good enough?* block.
