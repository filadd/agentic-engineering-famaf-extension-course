...
**Guest: Ale Silva on CCAD, LiteLLM and vLLM (~45 min)**
...
- **Track B, the advanced one, optional (~20-25 min)**: serve a small quantized model locally and compare three ways. **Scope and runtime are TBD with Agus** — [Raschka's local-coding-agents walkthrough](https://magazine.sebastianraschka.com/p/using-local-coding-agents) is the setup reference and uses Ollama. Nobody's outcome depends on finishing it.
- **Extension** for whoever finished Session 5: point their own loop at the same endpoint. One base URL, no tunnel.

> **Timing doesn't fit, and with a 45-minute guest slot it no longer fits even squeezed.** The full plan runs ~3 h 10 against a 2 h slot, and the tightest honest 2 h variant lands at 121 minutes with no break and the theory cut to the bone. `sessions/session-6/INSTRUCTOR.md` carries the arithmetic and three ways out — run longer, shorten the guest slot, or cut the hands-on below 35 min and lose the open-vs-hosted comparison. **It's a decision to take before class**, and it's tied to the open question about this session's duration.
...
- [Pi + llama.cpp](https://pi.dev/docs/latest/llama-cpp) — the optional local track as currently written. `/login llama.cpp` → `/llama` → `/model`.
